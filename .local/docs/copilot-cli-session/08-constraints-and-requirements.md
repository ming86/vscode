...
| SCP-03 | **Two palettes only.** Dark Modern and Light Modern, matching VS Code's defaults. No arbitrary theme support. Three selection modes: explicit light, explicit dark, or system-follow (resolves via `prefers-color-scheme` to one of the two palettes). | Shiki, CodeMirror 6, and the CSS token system all need consistent theme data. Supporting arbitrary themes requires a theme compilation pipeline. Two palettes are sufficient for the use case. | Theme toggle: dark / light / system. CSS custom properties map to VS Code's Dark Modern and Light Modern token values. `system` resolves to light or dark at runtime via `matchMedia('(prefers-color-scheme: dark)')`. |
