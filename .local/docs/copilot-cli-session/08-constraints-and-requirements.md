...
sequenceDiagram
    ...
    API->>SDK: getSession(id, autoRestore=true)
...
| SDK-05 | **`sessionCapabilities` gates agent behavior.** The `plan-mode`, `ask-user`, `interactive-mode`, `memory`, `cli-documentation`, and `system-notifications` capabilities must be declared at session creation. | Without them, the agent may refuse to invoke tools, ask questions, or produce plans. | Session creation must include the full capability set. Omission causes silent feature degradation. |
...
| OPS-11 | **`events.jsonl` can grow to several MB for long sessions.** The backend must handle large files without blocking the event loop. | Reading a 10MB file synchronously blocks the Node.js event loop for 50–100ms. | Use async file operations. Stream-parse `events.jsonl` for listing operations. The SDK's `autoRestore` handles replay — don't re-implement it. |
